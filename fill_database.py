...
from bookings.models import Table, Dish, Reservation, ReservationDish, UserProfile
...
# 3. Получение или создание пользователей-клиентов
print("\n3. Создание пользователей-клиентов...")
test_users_data = [
    {'username': 'client1', 'email': 'client1@example.com', 'first_name': 'Иван', 'last_name': 'Иванов'},
    {'username': 'client2', 'email': 'petrov@example.com', 'first_name': 'Петр', 'last_name': 'Петров'},
    {'username': 'client3', 'email': 'sidorova@example.com', 'first_name': 'Мария', 'last_name': 'Сидорова'},
    {'username': 'client4', 'email': 'kozlov@example.com', 'first_name': 'Алексей', 'last_name': 'Козлов'},
]
...
client_user = all_users[0]  # Используем первого пользователя для тестов
...
# Бронирования в прошлом (завершенные) - создаем напрямую без валидации
...
try:
    with connection.cursor() as cursor:
        cursor.execute("""
            INSERT INTO bookings_reservation (user_id, table_id, guests_count, start_time, end_time, created_at)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, [
            client_user.id, tables[2].id, 3,
            past_date.replace(hour=12, minute=0),
            past_date.replace(hour=13, minute=30),
            timezone.now()
        ])
    past_reservation = Reservation.objects.filter(user=client_user, table=tables[2], start_time__lt=timezone.now()).order_by('-start_time').first()
    if past_reservation:
...
print(f"  Пользователей-клиентов: {User.objects.filter(profile__role='client').count()}")
